...
默认模式的执行校验允许站点配额超限：实际到达时没有空位就由
`ChargingStation.start_charging` 加入队列，并累计 NYC 等待时间与等待惩罚，
不因站点超容量中断训练。每车恰好一个动作、订单容量和图数据一致性仍需满足。
该排队许可保存为图快照的 `allow_charging_queue`，回放不读取后续改变的环境开关。
Conservative 模式保持原有 wait 生成与严格容量校验。
