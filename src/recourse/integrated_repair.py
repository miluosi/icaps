...
def build_stage_graph(env, vehicle_ids, *, stage, state, initial=None, candidates=None):
    ...
    graph = StateSnapshotBuilder.feasible_graph_from_matrix(
        ...
        solver_backend='primal_dual', state=state)
    if stage == 0:
...
def simulate_integrated_repair(env, agents=None, current_requests=None, rebalance=True):
    started = time.perf_counter()
    RecourseTargetBuilder.validate_variant(getattr(env, 'recourse_variant', 'legacy'), 'integrated_repair')
    env.decision_mode = 'integrated_repair'
    env._leader_is_ev = None
    env._integrated_repair_solver_seconds = 0.
    pending = env._begin_joint_collection('integrated_repair')
    ...
    initial = build_stage_graph(env, ids, stage=0, state=initial_state)
    ...
    labels = {rid: 'rejected' if rid in rejected else 'unoffered' for rid in candidates}
    residual_state = StateSnapshotBuilder.build(env, request_labels=labels)
    repair = build_stage_graph(env, sorted(held), stage=2, state=residual_state, initial=initial, candidates=candidates)
    env._last_feasible_graph_snapshot = repair
    feasible_requests = {e.request_id for e in repair.edges if e.request_id is not None}
    for rid, category in labels.items():
        env.request_lifecycle.mark_residual(rid, epoch_id=initial_state.epoch_id, category=category,
                                           eligible=rid in feasible_requests, repair_architecture='integrated_repair')
    execute_commits(env, repair, actions, stores, stores_ev)
    for edge in _selected(repair):
        if edge.action_type == ActionType.SERVICE and env.vehicles[edge.vehicle_id].get('assigned_request') == edge.request_id:
            env.request_lifecycle.record_integrated_repair_assignment(
                edge.request_id, vehicle_id=edge.vehicle_id, epoch_id=initial_state.epoch_id)
    _fallback(env, actions)
    if pending:
        pending.ev_stage_graph, pending.aev_stage_graph = initial, repair
        pending.ev_joint_action, pending.aev_joint_action = JointActionSnapshot.from_graph(initial), JointActionSnapshot.from_graph(repair)
        ...
        pending.repair_candidate_request_ids = tuple(sorted(candidates))
    ...
    additions = dict(initial_integrated_ev_offer_count=sum(e.vehicle_type == 1 and e.action_type == ActionType.SERVICE and not dict(e.metadata).get('continuing') for e in _selected(initial)),
                     ...
                     repair_candidate_rejected_count=len(candidates & rejected),
                     repair_candidate_unassigned_count=len(candidates - rejected),
                     samitha_repair_assignment_count=sum(e.action_type == ActionType.SERVICE for e in _selected(repair)),
